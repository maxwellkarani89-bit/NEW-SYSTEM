...
def detailed_analysis():
    ...
    for base, quote in ALL_PAIRS:
        ...
        # Check if this is a standalone asset (XAU or BTC)
        is_standalone = base in ["XAU", "BTC"] and quote == "USD"
        
        try:
            if is_standalone:
                # ----- STANDALONE LOGIC FOR XAU/BTC -----
                # For standalone assets, don't subtract USD scores
                
                # Get asset's own economic scores only (no USD comparison)
                base_scores = get_currency_indicator_scores(base)
                
                # Economic indicators - use raw base scores (not normalized against USD)
                def get_raw_ind_score(ind_name):
                    # Returns 1, 0, -1 based on the asset's own indicator surprise
                ...
                # COT: Use asset's own COT only (no USD subtraction)
                base_cot = COTData.query.filter_by(currency=base).first()
                cot_net_dir = 1 if (base_cot.net_position if base_cot else 0) > 0 else (-1 if (base_cot.net_position if base_cot else 0) < 0 else 0)
                cot_mom_dir = 1 if (base_cot.weekly_change if base_cot else 0) > 0 else (-1 if (base_cot.weekly_change if base_cot else 0) < 0 else 0)
                cot_align = 2 if (cot_net_dir > 0 and cot_mom_dir > 0) else (-2 if (cot_net_dir < 0 and cot_mom_dir < 0) else 0)
                
                # Retail sentiment (already standalone)
                crowd = get_retail_sentiment_score(pair)
                
                # Technical (already standalone)
                yf_symbol = SYMBOL_MAPPING.get(pair, pair.replace('/', '') + '=X')
                trend = get_technical_directional_score(yf_symbol)
                
                # Seasonality (already standalone)
                season = get_seasonality_directional_score(pair)
                
                # Calculate total score (sum of all contributions, no USD subtraction)
                ...
                # ----- NORMAL PAIR LOGIC (existing code) -----
                weighted = calculate_weighted_pair_score(pair, base, quote)
                total_score = weighted["total_raw"]
                overall_bias, _, _, _ = classify_bias(total_score)
                ...
                def ind_pair_norm(ind_name):
                    b = base_scores.get(ind_name, 0)
                    q = quote_scores.get(ind_name, 0)
                    return normalize_pair_score(b, q)
